...
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const existingSelected = normalizeModelId(project.preferredCli ?? 'codex', project.selectedModel ?? undefined);
    ...
    // Helper: start preview only after AI has successfully generated code
    const startPreviewIfNeeded = () => {
      try {
    ...
    const isBypass = finalInstruction.trim().startsWith('bash:') || finalInstruction.trim().startsWith('mock:');
    if (isInitialPrompt && !isBypass) {
      const executor =
        cliPreference === 'codex'
      ...
      executor(
        ...
        // Only start preview after AI has successfully generated code
        startPreviewIfNeeded();
      }).catch((error) => {
        console.error('[API] Failed to initialize project:', error);
        if (requestId) {
          markUserRequestAsFailed(requestId, error instanceof Error ? error.message : 'Execution failed').catch(console.error);
          streamManager.publish(project_id, {
            type: 'status',
            data: { status: 'error', message: error instanceof Error ? error.message : 'Execution failed', requestId }
          });
        }
      });
    } else {
