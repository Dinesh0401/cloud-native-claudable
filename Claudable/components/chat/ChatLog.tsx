...
export default function ChatLog(props: ChatLogProps) {
  ...
  // Use refs for callbacks to prevent infinite dependency loops
  const onSessionStatusChangeRef = useRef(onSessionStatusChange);
  const onSseFallbackActiveRef = useRef(onSseFallbackActive);
  const onProjectStatusUpdateRef = useRef(onProjectStatusUpdate);
  ...
  useEffect(() => {
    ...
    onProjectStatusUpdateRef.current = onProjectStatusUpdate;
    startRequestRef.current = startRequest;
    completeRequestRef.current = completeRequest;
  }, [onSessionStatusChange, onSseFallbackActive, onProjectStatusUpdate, startRequest, completeRequest]);
  ...
  const handleRealtimeStatus = useCallback(
    (status: string, payload?: RealtimeStatus | Record<string, unknown>, requestId?: string) => {
      ...
      if (statusData?.status && statusData.message && status === 'project_status') {
        onProjectStatusUpdateRef.current?.(statusData.status, statusData.message);
      }
      ...
      if (requestKey && resolvedStatus === 'completed') {
        completeRequestRef.current?.(requestKey, true);
      }

      if (requestKey && resolvedStatus === 'error') {
        completeRequestRef.current?.(requestKey, false, statusData?.message);
      }
    },
    []
  );
  ...
  const { isConnected, isConnecting } = useWebSocket({
    ...
    onConnect: () => {
      ...
      onSseFallbackActiveRef.current?.(false);
  ...
  }, [projectId, enableSseFallback, handleRealtimeEnvelope, recoverMissingMessages]);
