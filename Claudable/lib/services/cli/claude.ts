...
- Prefer giving the user the live preview link that is actually running rather than written instructions.`,
    ...
    for await (const message of response) {
      ...
      if (message.type === 'system' && message.subtype === 'init') {
        ...
        // Detect auth failure messages from Claude CLI
        ...
        const isAuthFailure = AUTH_FAILURE_PATTERNS.some(pattern => pattern.test(content));

        if (isAuthFailure) {
          const authErrorMessage = `Claude Code CLI is not authenticated.\n\nTo fix this, run the following command in your terminal:\n\`\`\`\nnpx @anthropic-ai/claude-code auth login\n\`\`\`\n\nOr set the ANTHROPIC_API_KEY environment variable in your .env file.`;

          console.error('[ClaudeService] Auth failure detected:', content.substring(0, 200));

          // Save the error as a message so the user sees it in chat
          ...
          streamManager.publish(projectId, {
            type: 'message',
            data: serializeMessage(errorMsg, { requestId }),
          });

          // Also publish error status
          publishStatus('error', authErrorMessage);
          await safeMarkFailed(authErrorMessage);
