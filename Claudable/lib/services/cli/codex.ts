...
async function executeCodex(
  ...
  console.log('[CodexService] Triggering FastAPI Orchestrator', {
    projectId,
  ...
  if (!execRes.ok || !execRes.body) {
    throw new Error(`Failed to execute command: ${await execRes.text()}`);
  }

  // 3. Convert Web Stream to Node Stream for readline
  // @ts-ignore - Next.js/Node 18+ Web Streams interop
  const nodeStream = Readable.fromWeb(execRes.body as import('stream/web').ReadableStream);
  ...
  try {
    ...
    const isBypassCmd = instruction.trim().startsWith('bash:') || instruction.trim().startsWith('mock:');
      ...
      if (isBypassCmd) {
        ...
        streamManager.publish(projectId, {
          ...
          data: createRealtimeMessage({
            ...
            content: rawOutputAccumulator.trim(),
            metadata: { cli_type: 'codex', toolName: 'Bash', tool_name: 'Bash', command: instruction, status: 'running' },
            requestId,
            isStreaming: true,
          }),
        });
    ...
    if (isBypassCmd) {
      await dispatchToolMessage(
        projectId,
        rawOutputAccumulator.trim() || 'Command completed with no output.',
        {
          toolName: 'Bash',
          tool_name: 'Bash',
          command: instruction,
